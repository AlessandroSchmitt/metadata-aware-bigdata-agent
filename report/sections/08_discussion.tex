...
The experimental results show that metadata selection is not merely a prompt
compression technique. In the evaluated setting, relation-aware metadata
retrieval improves both efficiency and final Text-to-Spark-SQL correctness.

The complete-catalog baseline has access to all available metadata, yet it
achieves lower held-out result accuracy than the relation-aware
configuration. This indicates that providing more metadata is not necessarily
beneficial when the additional context contains semantically useful but
SQL-unsafe identifiers, unrelated schema elements, or competing concepts.

The proposed approach improves the quality of the generation context by
combining semantic relevance with structural information. The LLM receives
fewer metadata entities, but those entities are more directly connected to
the physical Spark SQL query that must be generated.

The improvement from 70\% result accuracy with the complete catalog to 80\%
with relation-aware retrieval supports this interpretation. The additional
increase to 85\% with one-shot repair shows that validator feedback can recover
a limited subset of remaining generation errors.


\subsection{Why the Complete Catalog Can Be Worse}
...
\subsection{Why the 8-to-16 Source Result Matters}
